...
//            mBroadcastInProgress = true;
    ...
    private void sendWakeUpBroadcast() {
        ...
        try {
            ...
            Slog.d(TAG, "ActivityManagerNative.getDefault().wakingUp();");
            // -- [framework] Add debug log.
    ...
    private void sendGoToSleepBroadcast(int reason) {
        ...
        try {
            // ++ [framework] Add debug log.
            Slog.d(TAG, "ActivityManagerNative.getDefault().goingToSleep();");
            // -- [framework] Add debug log.
            //++[framework] Monitor some function, peter li 20130325
    ...
    private void startMonitorFunction(String name) {
        ...
        if (DEBUG_ON) {
            Slog.d(TAG, "startMonitorFunction = " + name);
        }
        ...
        synchronized (sMonitorLock) {
            ...
            if (DEBUG_ON) {
                Slog.d(TAG, "monitor time = " + tmpTime);
            }
    ...
    private void endMonitorFunction(String name) {
        ...
        if (DEBUG_ON) {
            Slog.d(TAG, "endMonitorFunction = " + name);
        }
