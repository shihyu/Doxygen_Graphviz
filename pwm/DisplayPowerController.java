...
final class DisplayPowerController {
    ...
    // -- [framework] To support customized animation delay
        synchronized (mLock) {
            // ++ [framework] To support customized animation delay
            if (DEBUG_ON) {
                Slog.d(TAG, "sendUpdatePowerState: mCustomizeDuration=" + mCustomizeDuration + "->" + DURATION);
            }
...
//                    && mPowerRequest.screenState != DisplayPowerRequest.SCREEN_STATE_OFF) {
            ...
            if (mPowerRequest.useProximitySensor && mProximity != PROXIMITY_POSITIVE && mHtcAngleDetector != null) {
                if (mHtcAngleDetector.isTalkingDetected()) {
                    if (!mPowerRequest.forceScreenOn) {
                        if (DEBUG_ON) {
                            Slog.d(TAG, "Talking detected. turning off.");
                        }
                        sendOnAngleDetectorTalkingDetected();
                        setScreenOn(false);
                    } else {
                        if (DEBUG_ON) {
                            Slog.d(TAG, "Force screen on is set. disable detector.");
                        }
                    ...
                    if (DEBUG_ON) {
                        Slog.d(TAG, "Reading detected. turning on.");
                    }
                    if (mPowerRequest.forceScreenOn) {
                        mPowerRequest.forceScreenOn = false;
                    }
        ...
        // ++ [framework] Fix Google issue, allow screen on when p-sensor is always NEAR.
        if ((!mScreenOffBecauseOfProximity && (mHtcAngleDetector != null && !mHtcAngleDetector.isTalkingDetected())) || mPowerRequest.forceScreenOn) {
//        if (!mScreenOffBecauseOfProximity) {
        // -- [framework] Fix Google issue, allow screen on when p-sensor is always NEAR.
            if (wantScreenOn(mPowerRequest.screenState)) {
                // Want screen on.
    ...
    private void setProximitySensorEnabled(boolean enable) {
        if (enable) {
            if (!mProximitySensorEnabled) {
                ...
                if (DEBUG_ON) {
                    Slog.d(TAG, "Register Proximity Sensor Listener");
                }
            ...
            if (mProximitySensorEnabled) {
                ...
                if (DEBUG_ON) {
                    Slog.d(TAG, "Unregister Proximity Sensor Listener");
                }
    ...
    private void setLightSensorEnabled(boolean enable, boolean updateAutoBrightness) {
        if (enable) {
            if (!mLightSensorEnabled) {
                ...
                if (DEBUG_ON) {
                    Slog.d(TAG, "Register Light Sensor Listener");
                }
            ...
            if (mLightSensorEnabled) {
                ...
                if (DEBUG_ON) {
                    Slog.d(TAG, "Unregister Light Sensor Listener");
                }
                if (mHtcAutoBrightnessCtrl != null) {
...
//        mRecentLightSamples += 1;
    ...
    private void debounceButtonLight() {
        if (DEBUG_ON) {
            Slog.d(TAG, "debounceButtonLight mLightSensorEnabled = " + mLightSensorEnabled);
        }
        ...
        if (mLightSensorEnabled) {
            ...
            if (wantScreenOn(mPowerRequest.screenState)) {
                ...
                if (mHtcAutoBrightnessCtrl != null) {
                    if (DEBUG_ON) {
                        Slog.d(TAG, "Short Average = " + mRecentShortTermAverageLux + " ,Long Average = " + mRecentLongTermAverageLux);
                    }
    ...
    private void updateAutoBrightness(boolean sendUpdate, final int DURATION) {
        ...
        if (mScreenAutoBrightness != newScreenAutoBrightness || mScreenBrightnessRampAnimator.getCurrentBrightness() != newScreenAutoBrightness) {
            ...
            if (DEBUG || DEBUG_ON) {
                Slog.d(TAG, "updateAB:"
                        + " mSValue=" + mScreenAutoBrightness + "->" + newScreenAutoBrightness
                        + ", value=" + value
                        + ", gamma=" + gamma);
            }
            // -- [framework] Add DEBUG_ON flag, and add debug message.
    ...
    private final SensorEventListener mProximitySensorListener = new SensorEventListener() {
        ...
        public void onSensorChanged(SensorEvent event) {
            if (mProximitySensorEnabled) {
                ...
                Slog.d(TAG, "mProximityListener.onSensorChanged: " + (positive ? "NEAR" : "FAR"));
                // -- [framework] Add debug message
    ...
    protected boolean getProximitySensorActive() {
        ...
        Slog.d(TAG, "getProximitySensorActive: mProximity=" + mProximity);
        // -- [framework] Add Htc Log.
    ...
    protected int[] getLightSensorTableValues() {
        return mLux;
    }
    // -- [framework] for Sense 5 Car App
    ...
    protected final class DPCInternalAPI {
        ...
        protected void sendUpdatePowerState() {
            if (DEBUG_ON) {
                Slog.d(TAG, "DPCInternalAPI: sendUpdatePowerState");
            }
