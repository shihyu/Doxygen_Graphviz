...
//        mHandler = new Handler(true /*async*/);
    ...
    public void setButtonBrightness(int brightness) {
        if (DEBUG_ON) {
            Slog.d(TAG, "setButtonBrightness: mScreenOn = " + mScreenOn +" ,brightness=" + brightness);
        }
        if (mScreenOn) {
        ...
        public boolean setState(boolean on, int backlight) {
            synchronized (mLock) {
                ...
                // ++ [framework] Add button light control mechanism
                    ...
                    if (DEBUG_ON) {
                        Slog.d(TAG, "PhotonicModulator: bValue=" + mCurrentButtonValue + "->" + buttonValue);
                    }
            ...
            public void run() {
                ...
                for (;;) {
                    ...
                    if (on && mCurrentButtonValue != buttonValue && buttonValue >= 0) {
                        if (DEBUG_ON) {
                            Slog.d(TAG, "PhoMod Task: bValue=" + mCurrentButtonValue + "->" + buttonValue);
                        }
                        ...
                        if (DEBUG_ON) {
                            Slog.d(TAG, "PhoMod Task: turning off button light.");
                        }
                    ...
                    if (onChanged && on) {
                        if (DEBUG_ON) {
                            Slog.d(TAG, "PhoMod Task: unblankAllDisplays() ...");
                        }
                        mDisplayBlanker.unblankAllDisplays();
                        if (DEBUG_ON) {
                            Slog.d(TAG, "PhoMod Task: unblankAllDisplays() done.");
                        }
                    }
                    ...
                    if (onChanged && !on) {
                        if (DEBUG_ON) {
                            Slog.d(TAG, "PhoMod Task: blankAllDisplays() ...");
                        }
                        mDisplayBlanker.blankAllDisplays();
                        if (DEBUG_ON) {
                            Slog.d(TAG, "PhoMod Task: blankAllDisplays() done.");
                        }
                    }
                }
