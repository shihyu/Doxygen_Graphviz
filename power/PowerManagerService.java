...
public final class PowerManagerService extends IPowerManager.Stub
    ...
    private static final int MSG_USER_ACTIVITY_EVENT_TIMEOUT = 9;
    // -- [framework] Change dim timeout during Mirror or Miracast mode
    //++[framework] If the OOBE still keep in 1st page, then shutdown the devicePower
    ...
    // ++ [framework] Change dim timeout during Mirror or Miracast mode
    private static final int SCREEN_ENTER_DIM_IN_MIRROR_OR_MIRACAST_MODE = 5 *60 * 1000;
    // -- [framework] Change dim timeout during Mirror or Miracast mode
...
//    private long mTimeoutTaskAssignedTime = -1;
    ...
    // ++ [framework] Change dim timeout during Mirror or Miracast mode
    private static boolean forceScreenDim = false;
    // -- [framework] Change dim timeout during Mirror or Miracast mode

    //[++framework] Send intent to SmartDim when screen state is changed
    private int mLastUserActivityEvent = PowerManager.USER_ACTIVITY_EVENT_OTHER;
    //[--framework] Send intent to SmartDim when screen state is changed

    //[++framework] Add debug info for boot animation
    private int mBootAnimationCount = 0;
    //[--framework] Add debug info for boot animation

    //[++framework] Add by Peter li for Debug
    private int mWakefulnessLog = 0;
    private int mWakeLockSummaryLog = 0;
    //[--framework] Add by Peter li for Debug

    //++[framework] For max auto brightness feature
    private static final String SCREEN_AUTO_BRIGHTNESS_MAX = "screen_auto_brightness_max";

    // The screen auto-brightness setting, from 0% to 100%.
    // Use 100 if no value has been set.
    private int mScreenAutoBrightnessPercentageSetting = PowerManager.AUTO_BRIGHTNESS_MAX_PERCENTAGE_RANAGE;

    private int mScreenAutoBrightnessPercentageSettingDefault = PowerManager.AUTO_BRIGHTNESS_MAX_PERCENTAGE_RANAGE;

    // The screen auto brightness setting override from the settings application
    // to temporarily adjust the brightness until next updated,
    // Use -1 to disable.
    private int mTemporaryScreenAutoBrightnessPercentageSettingOverride = -1;
    //--[framework] For max auto brightness feature
    ...
    public void systemReady(TwilightService twilight, DreamManagerService dreamManager) {
        ...
        synchronized (mLock) {
            ...
            //++[framework] For max auto brightness feature
            mScreenAutoBrightnessPercentageSettingDefault = pm.getDefaultScreenAutoBrightnessPercentageSetting();
            //--[framework] For max auto brightness feature
            ...
            //[++framework] Use HTC BOOT COMPLETED intent instead of Google BOOT COMPLETED intent
            filter.addAction(com.htc.content.Intents.ACTION_HTC_BOOT_COMPLETED);
    ...
    private void updateSettingsLocked() {
        ...
        //++[framework] For max auto brightness feature
        final int oldScreenAutoBrightnessPercentageSetting = mScreenAutoBrightnessPercentageSetting;
        mScreenAutoBrightnessPercentageSetting = Settings.System.getIntForUser(resolver,
                SCREEN_AUTO_BRIGHTNESS_MAX, mScreenAutoBrightnessPercentageSettingDefault,
                UserHandle.USER_CURRENT);

        if (oldScreenAutoBrightnessPercentageSetting != mScreenAutoBrightnessPercentageSetting) {
            mTemporaryScreenAutoBrightnessPercentageSettingOverride = -1;
        }
        //--[framework] For max auto brightness feature
        ...
        if (DEBUG_ON) {
            Slog.d(TAG, "SettingsObserver:"
                    ...
                    + ", mScreenAutoBrightnessPercentageSetting=" + oldScreenAutoBrightnessPercentageSetting + "->" + mScreenAutoBrightnessPercentageSetting
                    + ", mTemporaryScreenAutoBrightnessPercentageSettingOverride=" + mTemporaryScreenAutoBrightnessPercentageSettingOverride);
        }
    ...
    private void addWakeLockToQueue(IBinder lock, int flags, String tag, String packageName, WorkSource ws, int uid, int pid) {
        WakeLock wakeLock;
    ...
    private void updateWakeLockWorkSourceFromQueue(IBinder lock, WorkSource ws) {
        int index = findWakeLockIndexLockedFromQueue(lock);

        if (index < 0) {
            if (DEBUG_ON) {
                Slog.w(TAG, "updateWLWSFQ not find");
            }
            return;
        }

        if (DEBUG_ON) {
            Slog.d(TAG, "updateWLWSFQ index=" + index);
        }

        WakeLock wakeLock = mTmpWakeLockPool.get(index);
    ...
    private void handleWakeLockFromQueue() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            try {
                acquireWakeLockInternal(wakeLock.mLock, wakeLock.mFlags, wakeLock.mTag, wakeLock.mPackageName, wakeLock.mWorkSource, wakeLock.mOwnerUid, wakeLock.mOwnerPid);
            } finally {
    ...
    public void acquireWakeLock(IBinder lock, int flags, String tag, String packageName,
        ...
        synchronized (mInitLock) {
            if (mInitReady == false) {
                addWakeLockToQueue(lock, flags, tag, packageName, ws, Binder.getCallingUid(), Binder.getCallingPid());
                return;
            }
        }
        // -- [framework] acquireWakeLock happened before PowerManagerService init.
    ...
    private void acquireWakeLockInternal(IBinder lock, int flags, String tag, String packageName,
        ...
        synchronized (mLock) {
            ...
            if ((wakeLock.mFlags & PowerManager.PROXIMITY_SCREEN_OFF_WAKE_LOCK) != 0) {
                if (DEBUG_ON) {
                    Slog.d(TAG, "acquireWL:p wl");
                }
                if (mDisplayPowerController != null) {
                    mDisplayPowerController.notifyReleaseProximityWakeLock(false, -1);
                }
                if (mDisplayPowerRequest != null) {
                    mDisplayPowerRequest.delayScreenOn = false;
                }
            }
            // -- [framework] Workaround for M7 p-sensor problem after ending call
...
//                    nativeAcquireCpuPerfWakeLock();
            ...
            if ((DEBUG_ON || true) && HtcPMSExtension.canPrintWakeLockLog(flags, tag)) {
...
//                    nativeAcquireCpuPerfWakeLock();
            ...
            if (DEBUG_ON || true) {
                ...
                if (!htcCpuCtrl.hasSameProperties(flags, tag, ws, uid, pid)) {
                    f = new Formatter();
                    f.format("releaseHCC(%x): %s %s 0x%x", 
                            htcCpuCtrl.hashCode(), typeInfo, new_tag, flags);
                    Slog.d(TAG, f.toString());
                }
                f = new Formatter();
                f.format("acquireHCC(%x): %s %s 0x%x %d %d",
    ...
    private void applyWakeLockFlagsOnAcquireLocked(WakeLock wakeLock) {
        if ((wakeLock.mFlags & PowerManager.ACQUIRE_CAUSES_WAKEUP) != 0
            ...
            if (isForceScreenOn() && wakeLock.mOwnerUid == Process.SYSTEM_UID && mDisplayPowerRequest != null && "keyguard".equals(wakeLock.mTag)) {
                if (DEBUG_ON) {
            ...
            if (mDisplayPowerController != null) {
                ...
                if (mDisplayPowerController.inEndCallMode() && wakeLock.mOwnerUid == Process.SYSTEM_UID && "keyguard".equals(wakeLock.mTag)) {
                    Slog.d(TAG, "Wake up requested from keyguard in End Call Mode.");
                    mDisplayPowerController.clearEndCallMode();
                }
            ...
            if (needTurnOnScreenByPowerKey() && wakeLock.mOwnerUid == Process.SYSTEM_UID && "keyguard".equals(wakeLock.mTag)) {
                if (mDisplayPowerRequest != null) {
                    if (DEBUG_ON) {
                        Slog.d(TAG, "Wake up requested from keyguard to turn on screen when screen is off.");
                    }
                    mDisplayPowerRequest.turnOnScreenOnce = true;
                }
    ...
    private void updateHtcCpuCtrlLevel(int flags) {
        ...
        switch (flags & PowerManager.WAKE_LOCK_LEVEL_MASK) {
            ...
            case PowerManager.CPU_PERF_WAKE_LOCK:
                if (freq_min_level != 0) {
                    if (freq_max_level != 0 && freq_max_level < freq_min_level) {
                        Slog.w(TAG, "Freq conflict in CPU_PERF:"
                                + "(" + freq_max_level
                                + "," + freq_min_level +")");
                ...
                if (num_min_level != 0) {
                    if (num_max_level != 0 && num_max_level < num_min_level) {
                        Slog.w(TAG, "Num conflict in CPU_PERF:"
                                + "(" + num_max_level
                                + "," + num_min_level + ")");
            ...
            case PowerManager.CPU_MIN_FREQ:
                ...
                if (freq_max_level != 0 && freq_max_level < freq_min_level) {
                    Slog.w(TAG, "Freq conflict in CPU_MIN_FREQ:"
                            + "(" + freq_max_level
                            + "," + freq_min_level + ")");
                    freq_min_level = freq_max_level;
                }
            ...
            case PowerManager.CPU_MAX_FREQ:
                ...
                if (freq_min_level != 0 && freq_max_level < freq_min_level) {
                    Slog.w(TAG, "Freq conflict in CPU_MAX_FREQ:"
                            + "(" + freq_max_level
                            + "," + freq_min_level + ")");
                    freq_min_level = freq_max_level;
                    nativeAcquireCpuFreqMinWakeLock(freq_min_level);
            ...
            case PowerManager.CPU_MIN_NUM:
                ...
                if (num_max_level != 0 && num_max_level < num_min_level) {
                    Slog.w(TAG, "Num conflict in CPU_MIN_NUM:"
                            + "(" + num_max_level
                            + "," + num_min_level + ")");
                    num_min_level = num_max_level;
                }
            ...
            case PowerManager.CPU_MAX_NUM:
                ...
                if (num_min_level != 0 && num_max_level < num_min_level) {
                    Slog.w(TAG, "Num conflict in CPU_MAX_NUM:"
                            + "(" + num_max_level
                            + "," + num_min_level + ")");
                    num_min_level = num_max_level;
                    nativeAcquireCpuNumMinWakeLock(num_min_level);
    ...
    private void releaseWakeLockInternal(IBinder lock, int flags) {
        synchronized (mLock) {
            ...
            // ++ [framework] Add WakeLock related logs to determine issue root cause more easily.
            if ((DEBUG_ON || true) && HtcPMSExtension.canPrintWakeLockLog(wakeLock.mFlags, wakeLock.mTag)) {
                // ++ [framework] Encrypt tag if neccessary in release rom.
                ...
                f.format("releaseWL(%x): %s %s %s 0x%x", wakeLock.hashCode(), typeInfo, flagInfo, new_tag, wakeLock.mFlags);
                Slog.d(TAG, f.toString());
            }
            // -- [framework] Add WakeLock related logs to determine issue root cause more easily.
    ...
    private void releaseHtcCpuCtrlInternal(IBinder lock, int flags) {
        synchronized (mLock) {
            ...
            if (DEBUG_ON || true) {
                ...
                Formatter f = new Formatter();
                f.format("releaseHCC(%x): %s %s 0x%x", htcCpuCtrl.hashCode(), typeInfo, new_tag, htcCpuCtrl.mFlags);
                Slog.d(TAG, f.toString());
            }
            // -- [framework] Add WakeLock related logs to determine issue root cause more easily.
...
//                }
    ...
    private void handleWakeLockDeath(WakeLock wakeLock) {
        synchronized (mLock) {
            ...
            if (index < 0) {
                ...
                if (mHtcWakeLockMonitor != null) {
                    try {
                        mHtcWakeLockMonitor.stopDeadRecord(wakeLock.mLock);
                    } catch (Exception e) {
                        if (DEBUG_ON) {
                            Slog.w(TAG, "stopDeadRecord Exception : ", e);
                        }
                    }
                }
                // -- [framework] Monitor wake lock behaviors
            ...
            if (DEBUG_ON || true) {
                ...
                String flagInfo = wakeLock.getLockFlagsString();
                Formatter f = new Formatter();
                f.format("handleWLDeath(%x): %s %s %s 0x%x", wakeLock.hashCode(), typeInfo, flagInfo, new_tag, wakeLock.mFlags);
                Slog.d(TAG, f.toString());
            }
            // -- [framework] Add WakeLock related logs to determine issue root cause more easily.
            ...
            if (mHtcWakeLockMonitor != null) {
                try {
                    ...
                    if (DEBUG_ON) {
                        Slog.w(TAG, "stopRecord Exception : ", ex);
                    }
                }
            }
            // -- [framework] Monitor wake lock behaviors
    ...
    private void handleHtcCpuCtrlDeath(HtcCpuCtrl htcCpuCtrl) {
        synchronized (mLock) {
            ...
            if (index < 0) {
                ...
                if (mHtcWakeLockMonitor != null) {
                    try {
                        mHtcWakeLockMonitor.stopDeadRecord(htcCpuCtrl.mLock);
                    } catch (Exception e) {
                        if (DEBUG_ON) {
                            Slog.w(TAG, "HCC stopDeadRecord Exception : ", e);
                        }
                    }
                }
                // -- [framework] Monitor wake lock behaviors
            ...
            if (DEBUG_ON || true) {
                ...
                String typeInfo = HtcPMSExtension.getWakeLockTypeInfo(htcCpuCtrl.mFlags);
                Formatter f = new Formatter();
                f.format("handleHCCDeath(%x): %s %s 0x%x", htcCpuCtrl.hashCode(), typeInfo, new_tag, htcCpuCtrl.mFlags);
                Slog.d(TAG, f.toString());
            }
            // -- [framework] Add WakeLock related logs to determine issue root cause more easily.
            ...
            if (mHtcWakeLockMonitor != null) {
                try {
                    ...
                    if (DEBUG_ON) {
                        Slog.w(TAG, "HCC stopRecord Exception : ", ex);
                    }
                }
            }
            // -- [framework] Monitor wake lock behaviors
    ...
    public void updateWakeLockWorkSource(IBinder lock, WorkSource ws) {
        ...
        synchronized (mInitLock) {
            if (mInitReady == false) {
                updateWakeLockWorkSourceFromQueue(lock, ws);
                return;
            }
        }
        // -- [framework] acquireWakeLock happened before PowerManagerService init.
    ...
    // -- [framework] Overload function with 'force' variable
        ...
        // ++ [framework] Change dim timeout during Mirror or Miracast mode
        if (event != PowerManager.USER_ACTIVITY_EVENT_OTHER) {
            updateUserActivityEventLocked();
        }
        // -- [framework] Change dim timeout during Mirror or Miracast mode
        ...
        if (mScreenTimeout_Once > 0) {
            if (DEBUG_ON) {
                Slog.d(TAG, "clear Timeout_Once(" + mScreenTimeout_Once + ")");
            }
...
//            if (eventTime > mLastUserActivityTime) {
    ...
    // ++ [framework] Force screen on by pressing power key.
    private boolean isForceScreenOn() {
        boolean forceScreenOn = false;

        try {
            if (mProximityPositive || mADTalkingDetected) {
                forceScreenOn = true;
                if (DEBUG_ON) {
                    Slog.d(TAG, "case1 fso ");
                }
            } else if (((mWakeLockSummary & WAKE_LOCK_PROXIMITY_SCREEN_OFF) != 0)
                        && (mActualScreenOn == false)
                        && (mWakefulness == WAKEFULNESS_AWAKE)) {
                forceScreenOn = true;
                if (DEBUG_ON) {
                    Slog.d(TAG, "case2 fso ");
                }
            }
        } catch (Exception e) {
            Slog.d(TAG, "isForceScreenOn exception : " + e);
        }

        return forceScreenOn;
    }

    private void clearForceScreenOnFlag() {
        if (mDisplayPowerRequest != null) {
            mDisplayPowerRequest.forceScreenOn = false;
        }
    }
    ...
    private void wakeUpFromNative(long eventTime) {
        ...
        // ++ [framework] Fix Google issue, allow screen on when p-sensor is always NEAR.
        if (mDisplayPowerRequest != null && isForceScreenOn()) {
            if (DEBUG_ON) {
        ...
        // ++ [framework] Workaround for M7 p-sensor problem after ending call
        if (mDisplayPowerController != null && mDisplayPowerController.inEndCallMode()) {
            if (DEBUG_ON) {
                Slog.d(TAG, "Wake up requested from native in End Call Mode.");
            }
            mDisplayPowerController.clearEndCallMode();
        }
        // -- [framework] Workaround for M7 p-sensor problem after ending call
        ...
        // ++ [framework] Workaround for turning on screen when screen is off.
        if (needTurnOnScreenByPowerKey()) {

            if (mDisplayPowerRequest != null) {
                if (DEBUG_ON) {
                    Slog.d(TAG, "Wake up requested from native to turn on screen when screen is off.");
                }
                mDisplayPowerRequest.turnOnScreenOnce = true;
            }
    ...
    private boolean wakeUpNoUpdateLocked(long eventTime) {
        ...
        if ((eventTime < mLastSleepTime || mWakefulness == WAKEFULNESS_AWAKE
                || !mBootCompleted || !mSystemReady) && !mDisplayPowerRequest.forceScreenOn && !mDisplayPowerRequest.turnOnScreenOnce) {
//        if (eventTime < mLastSleepTime || mWakefulness == WAKEFULNESS_AWAKE
//                || !mBootCompleted || !mSystemReady) {
    ...
    private void updateIsPoweredLocked(int dirty) {
        if ((dirty & DIRTY_BATTERY_STATE) != 0) {
            ...
            if (DEBUG || DEBUG_ON) {
            // -- [framework] Add DEBUG_ON.
                Slog.d(TAG, "updateIsPowered(" + wasPowered
                        + "," + mIsPowered
                        + "," + oldPlugType
                        + "," + mPlugType
                        + "," + mBatteryLevel + ")");
...
//                        }
            ...
            // -- [framework] Add DEBUG_ON flag.
                if ((mWakefulness != mWakefulnessLog) || (mWakeLockSummary != mWakeLockSummaryLog)) {
                    Slog.d(TAG, "updateWLS"
                            + "(" + wakefulnessToString(mWakefulness)
                            + ",0x" + Integer.toHexString(mWakeLockSummary) + ")");
                    mWakefulnessLog = mWakefulness;
                    mWakeLockSummaryLog = mWakeLockSummary;
                }
            }
        }
    }
    ...
    private void updateUserActivitySummaryLocked(long now, int dirty) {
        ...
        if ((dirty & (DIRTY_USER_ACTIVITY | DIRTY_WAKEFULNESS | DIRTY_SETTINGS)) != 0) {
            ...
            if (DEBUG_SPEW || DEBUG_ON) {
            // -- [framework] Add DEBUG_ON flag
                Slog.d(TAG, "updateUAS(" + wakefulnessToString(mWakefulness)
                        + ",0x" + Integer.toHexString(mUserActivitySummary)
                        // ++ [framework] Add debug message.
                        + "," + now
                        + "," + dirty
                        + "," + screenDimDuration
    ...
    // ++ [framework] Change dim timeout during Mirror or Miracast mode
    private void handleUserActivityEventTimeout() { // runs on handler thread
        if (DEBUG_ON) {
            Slog.d(TAG, "handleUAET");
        }
        synchronized (mLock) {
            forceScreenDim = true;
        }
    ...
    private void updateUserActivityEventLocked() {
        if (DEBUG_ON) {
            Slog.d(TAG, "udaUAEL");
        }

        try {
            forceScreenDim = false;
            mHandler.removeMessages(MSG_USER_ACTIVITY_EVENT_TIMEOUT);

            if (isMirrorModeTurnedOn() || isMiracastModeTurnedOn()) {

                if (DEBUG_ON) {
                    Slog.d(TAG, "At Mirror or Miracast mode");
                }

                Message msg = mHandler.obtainMessage(MSG_USER_ACTIVITY_EVENT_TIMEOUT);
                msg.setAsynchronous(true);
                mHandler.sendMessageDelayed(msg, SCREEN_ENTER_DIM_IN_MIRROR_OR_MIRACAST_MODE);
            }

        } catch (Exception e) {
            Slog.w(TAG, "Exception for updateUserActivityEventLocked");
        }

    }

    private boolean isMirrorModeTurnedOn() {
        if (mHtcDongleMode != null && mHtcDongleMode.isMirrorMode()) {
            return true;
        }
        return false;
    }

    private boolean isMiracastModeTurnedOn() {
        if (mHtcDongleMode != null && mHtcDongleMode.isMiracastMode()) {
            return true;
        }
        return false;
    }
    ...
    private int adjustHtcScreenOffDelay(int totalDelay) {
        ...
        if ((-1) == mScreenOffTimeoutSetting) {
            ...
            if (DEBUG_ON) {
                Slog.d(TAG, "adjustDelay[NTF]=" + adjTotalDelay);
            }
        }
        ...
        try {
            if (HtcPMSExtension.getHtcCustomizationReader("Android_Core_Framework").readBoolean("support_3lm", false) &&
                ...
                if (DEBUG_ON) {
                    Slog.d(TAG, "adjustDelay[3LM]=" + adjTotalDelay);
                }
            }
        } catch (Exception e) {
            if (DEBUG_ON) {
        ...
        if (mbEnableDeskDockBehavior) {
            ...
            if (mDeskModeEnabled && mIsPowered && mScreenOffDelayForDeskDock != -1) {
                ...
                if (DEBUG_ON) {
                    Slog.d(TAG, "adjustDelay[DeskDock]=" + adjTotalDelay);
                }
            }
        }
        // -- Greg Tsai (2011/06/21), support new desk dock behaviors: keep screen on when desk mode is on with charging.
        ...
        if (isDongleModeTurnedOn()) {
            adjTotalDelay = mMaximumScreenOffTimeoutFromDeviceAdmin;
            if (DEBUG_ON) {
                Slog.d(TAG, "adjustDelay[DongleMode]=" + adjTotalDelay);
            }
        }
        // -- [framework] Add screen timeout mechanism when dongle mode is turned on
    ...
    private int adjustHtcScreenDimDuration(final int screenOffTimeout) {
        ...
        if (mHtcPMSExtension != null && mHtcPMSExtension.isAMOLED()) {
            if ((-1) == mScreenOffTimeoutSetting) {
                ...
                if (DEBUG_ON) {
                    Slog.d(TAG, "adjustDim[AMOLED]=" + adjDimDuration);
                }
            }
        }
        ...
        if (isDongleModeTurnedOn()) {
            ...
            if (DEBUG_ON) {
                Slog.d(TAG, "adjustDim[DongleMode]=" + adjDimDuration);
            }
        }
        // -- [framework] Add screen dim mechanism when dongle mode is turned on
    ...
    public void customizeScreenOffDelay_Once(final int screenOffDelay, final boolean calcRemaining) {
        mContext.enforceCallingOrSelfPermission(com.htc.framework.Manifest.permission.APP_PLATFORM, "customizeScreenOffDelay_Once"); //[framework] add permission to protect, freya, 20131223
        if (DEBUG_ON) {
            Slog.d(TAG, "Delay_O(" + screenOffDelay + ", " + calcRemaining
                    + "," + Binder.getCallingPid()
                    + "," + Binder.getCallingUid() + ")");
        }
        synchronized (mLock) {
    ...
    public void customizeScreenOffDelay_Always(final int screenOffDelay, final boolean calcRemaining) {
        mContext.enforceCallingOrSelfPermission(com.htc.framework.Manifest.permission.APP_PLATFORM, "customizeScreenOffDelay_Always"); //[framework] add permission to protect, freya, 20131223
        if (DEBUG_ON) {
            Slog.d(TAG, "Delay_A(" + screenOffDelay + ", " + calcRemaining
                    + "," + Binder.getCallingPid()
                    + "," + Binder.getCallingUid() + ")");
        }
        synchronized (mLock) {
    ...
    public void customizeScreenOffDelay_System(final int screenOffDelay, final boolean calcRemaining) {
        mContext.enforceCallingOrSelfPermission(com.htc.framework.Manifest.permission.APP_PLATFORM, "customizeScreenOffDelay_System"); //[framework] add permission to protect, freya, 20131223
        if (DEBUG_ON) {
            Slog.d(TAG, "Delay_S(" + screenOffDelay + ", " + calcRemaining
                    + "," + Binder.getCallingPid()
                    + "," + Binder.getCallingUid() + ")");
        }
        synchronized (mLock) {
    ...
    private void customizeScreenOffDelayLocked(final int flag, final int screenOffDelay, final boolean calcRemaining) {
        ...
        if (screenOffDelay < 0) {
            ...
            if (DEBUG_ON) {
                Slog.d(TAG, "ScreenOffDelay: reset("
                        + "0x" + Integer.toHexString(flag)
                        + "," + mScreenTimeout_Once
                        + "," + mScreenTimeout_Always
                        + "," + mScreenTimeout_System + ")");
            }
...
//                    setTimeoutLocked(SystemClock.uptimeMillis(), SCREEN_BRIGHT);
            ...
            // Case: Previous Timeout = New Timeout
                ...
                if (DEBUG_ON) {
                    Slog.d(TAG, "ScreenOffDelay: Same. Skip");
                }
            }
        }
        ...
        if (DEBUG_ON) {
            Slog.d(TAG, "ScreenOffDelay:("
                    + "0x" + Integer.toHexString(flag)
                    + "," + screenOffDelay
                    + "," + calcRemaining
                    + "," + prev_once + "->" + mScreenTimeout_Once
                    + "," + prev_always + "->" + mScreenTimeout_Always
                    + "," + prev_system + "->" + mScreenTimeout_System
                    + "," + wakefulnessToString(mWakefulness) + ")");
        }
...
//            setTimeoutLocked(SystemClock.uptimeMillis(), SCREEN_BRIGHT);
    ...
    private void updateDisplayPowerStateLocked(int dirty) {
        if ((dirty & (DIRTY_WAKE_LOCKS | DIRTY_USER_ACTIVITY | DIRTY_WAKEFULNESS
            ...
            //[++framework] Send intent to SmartDim when screen state is changed
            int preScreenState = mDisplayPowerRequest.screenState;
            //[--framework] Send intent to SmartDim when screen state is changed
            ...
            //++[framework] For max auto brightness feature
            int screenAutoBrightnessPercentage = mScreenAutoBrightnessPercentageSettingDefault;
            //--[framework] For max auto brightness feature
            ...
            if (autoBrightness) {
                ...
                //++[framework] For max auto brightness feature
                if (isValidAutoBrightnessPercentage(
                        mTemporaryScreenAutoBrightnessPercentageSettingOverride)) {
                    screenAutoBrightnessPercentage =
                            mTemporaryScreenAutoBrightnessPercentageSettingOverride;
                } else if (isValidAutoBrightnessPercentage(
                        mScreenAutoBrightnessPercentageSetting)) {
                    screenAutoBrightnessPercentage = mScreenAutoBrightnessPercentageSetting;
                }
                //--[framework] For max auto brightness feature
            }
            ...
            //++[framework] For max auto brightness feature
            screenAutoBrightnessPercentage = Math.max(Math.min(screenAutoBrightnessPercentage,
                    PowerManager.AUTO_BRIGHTNESS_MAX_PERCENTAGE_RANAGE), PowerManager.AUTO_BRIGHTNESS_MIN_PERCENTAGE_RANAGE);
            //--[framework] For max auto brightness feature
            ...
            // -- [framework] to determine whether the screen brightness came from override
            //++[framework] For max auto brightness feature
            mDisplayPowerRequest.screenAutoBrightnessPercentage = screenAutoBrightnessPercentage;
            //--[framework] For max auto brightness feature
            ...
            //[++framework] Send intent to SmartDim when screen state is changed
            sendScreenStateChangeLocked(preScreenState, newScreenState);
            //[--framework] Send intent to SmartDim when screen state is changed
    ...
    //[++framework] Send intent to SmartDim when screen state is changed
    private void sendScreenStateChangeLocked(final int preState, final int newState) {
        if (preState != newState) {
            boolean sendIntent = false;
            int screenChange = 0;

            if (DEBUG_ON) {
                Slog.d(TAG, "sendSSCL: (" + preState + "," + newState + ")");
            }

            if (preState == DisplayPowerRequest.SCREEN_STATE_BRIGHT && newState == DisplayPowerRequest.SCREEN_STATE_DIM) {
                sendIntent = true;
                screenChange = PowerManager.SCREEN_STATE_CHANGE_FROM_BRIGHT_TO_DIM;
            } else if (preState == DisplayPowerRequest.SCREEN_STATE_DIM && newState == DisplayPowerRequest.SCREEN_STATE_BRIGHT) {
                sendIntent = true;
                screenChange = PowerManager.SCREEN_STATE_CHANGE_FROM_DIM_TO_BRIGHT;
            }

            if (sendIntent && mHtcPMSExtension != null) {
                mHtcPMSExtension.sendScreenStateChange(screenChange, mLastUserActivityTime, mLastUserActivityEvent);
            }
    ...
    private int getDesiredScreenPowerStateLocked() {
        ...
        if ((mWakeLockSummary & WAKE_LOCK_SCREEN_BRIGHT) != 0
            ...
            // ++ [framework] Change dim timeout during Mirror or Miracast mode
            if ((isMirrorModeTurnedOn() || isMiracastModeTurnedOn()) && forceScreenDim == true) {
        ...
        public void onProximityNegative() {
            synchronized (mLock) {
                ...
                if (mDisplayPowerRequest != null && mDisplayPowerRequest.forceScreenOn) {
                    if (DEBUG_ON) {
                        Slog.d(TAG, "onPN:disable force screen on");
                    }
                    mDisplayPowerRequest.forceScreenOn = false;
                }
                // ++ [framework] Workaround for M7 p-sensor problem.
...
//                        PowerManager.USER_ACTIVITY_EVENT_OTHER, 0, Process.SYSTEM_UID);
        ...
        public void onAngleDetectorReadingDetected() {
            synchronized (mLock) {
                ...
                if (mDisplayPowerRequest != null && mDisplayPowerRequest.forceScreenOn) {
                    if (DEBUG_ON) {
                        Slog.d(TAG, "onADRD:disable force screen on");
                    }
                    mDisplayPowerRequest.forceScreenOn = false;
                }
                // -- [framework] Fix Google issue, allow screen on when p-sensor is always NEAR.
    ...
    private void checkIfBootAnimationFinished() {
        ...
        if (SystemService.isRunning(BOOT_ANIMATION_SERVICE)) {
            ...
            if (DEBUG_ON) {
                mBootAnimationCount++;
                if (mBootAnimationCount > 100) {
                    mBootAnimationCount = 0;
                    Slog.d(TAG, "Boot Animation 100");
                }
            }
            return;
        }
        ...
        if (DEBUG_ON) {
            Slog.d(TAG, "Final boot animation Count = " + mBootAnimationCount);
            mBootAnimationCount = 0;
        }
    ...
    private void handleClearScreenTimeoutTask(int timeout, int calcRemaining) {
        if (DEBUG_ON) {
            Slog.d(TAG, "handleClearTask(" + timeout + "," + calcRemaining + ")");
        }
        mHandler.removeCallbacks(mClearScreenTimeoutTask);
        mHandler.removeMessages(MSG_SEND_CLEAR_SCREEN_TIMEOUT_TASK);
    ...
    private void sendClearScreenTimeoutTask(int timeout, int calcRemaining) {
        if (DEBUG_ON) {
            Slog.d(TAG, "sendClearTask(" + timeout + "," + calcRemaining + ")");
            Slog.d(TAG, "Post(" + mLastUserActivityTime + "," + SystemClock.uptimeMillis() + ")");
        }
        mHandler.removeCallbacks(mClearScreenTimeoutTask);
...
//        mHandler.sendMessage(msg);
    ...
    public void setMaximumScreenOffTimeoutFromDeviceAdmin(int timeMs) {
        // ++ [framework] Add debug message.
        if (DEBUG_ON) {
            Slog.d(TAG, "setMSOTFromDeviceAdmin:"
                    + "(" + timeMs
                    + "," + Binder.getCallingPid()
                    + "," + Binder.getCallingUid() + ")");
        }
...
//        }
    ...
    public void setTemporaryScreenBrightnessSettingOverride(int brightness) {
        ...
        // ++ [framework] Add debug message.
        if (DEBUG_ON) {
            Slog.d(TAG, "setTSBSO:(" + brightness
                    + "," + Binder.getCallingPid()
                    + "," + Binder.getCallingUid() + ")");
        }
        // -- [framework] Add debug message.
    ...
    public void lowLevelShutdown_system() {
        mContext.enforceCallingOrSelfPermission(com.htc.framework.Manifest.permission.APP_PLATFORM, "lowLevelShutdown_system"); //[framework] add permission to protect, freya, 20131223
    ...
    public void lowLevelReboot_system(String reason) {
        mContext.enforceCallingOrSelfPermission(com.htc.framework.Manifest.permission.APP_PLATFORM, "lowLevelReboot_system"); //[framework] add permission to protect, freya, 20131223
        int uid = Binder.getCallingUid();
        if (uid != Process.SYSTEM_UID) {
    ...
    public boolean wipeSystem(byte[] imei, byte[] encMsg) {
        mContext.enforceCallingOrSelfPermission(com.htc.framework.Manifest.permission.APP_PLATFORM, "wipeSystem"); //[framework] add permission to protect, freya, 20131223
        try {
            ...
            if (imei == null || encMsg == null) {
                if (DEBUG_ON) {
                    Slog.d(TAG, "wipeSystem: Null found.(" + imei + "," + encMsg + ")");
                }
                return false;
            }

            if (HtcBuildFlag.Htc_SECURITY_DEBUG_flag) {
                Slog.d(TAG, "wipeSystem:(" + new String(imei) + "," + new String(encMsg)
                        + "," + Binder.getCallingPid()
                        + "," + Binder.getCallingUid() + ")");
            }
    ...
    protected void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        synchronized (mLock) {
            ...
            //++[framework] For max auto brightness feature
            pw.println("  mScreenAutoBrightnessPercentageSetting=" + mScreenAutoBrightnessPercentageSetting);
            pw.println("  mScreenAutoBrightnessPercentageSettingDefault=" + mScreenAutoBrightnessPercentageSettingDefault);
            pw.println("  mTemporaryScreenAutoBrightnessPercentageSettingOverride="
                    + mTemporaryScreenAutoBrightnessPercentageSettingOverride);
            //--[framework] For max auto brightness feature
    ...
    private final class BootCompletedReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (DEBUG_ON) {
                Slog.d(TAG, "BootCompletedReceiver E");
            }
        }
    }
    ...
    private final class PowerManagerHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                //[--framework] need to post ClearScreenTimeoutTask to clear once flag
                // ++ [framework] Change dim timeout during Mirror or Miracast mode
                case MSG_USER_ACTIVITY_EVENT_TIMEOUT:
                    handleUserActivityEventTimeout();
                    break;
                // -- [framework] Change dim timeout during Mirror or Miracast mode
                //++[framework] If the OOBE still keep in 1st page, then shutdown the devicePower
...
//    }
    ...
    public int fetchCurrentBrightnessValue() {
        mContext.enforceCallingOrSelfPermission(com.htc.framework.Manifest.permission.APP_PLATFORM, "fetchCurrentBrightnessValue"); //[framework] add permission to protect, freya, 20131223
        if (mDisplayPowerController != null) {
    ...
    public boolean getProximitySensorActive() {
        mContext.enforceCallingOrSelfPermission(com.htc.framework.Manifest.permission.APP_PLATFORM, "getProximitySensorActive"); //[framework] add permission to protect, freya, 20131223
        if (mDisplayPowerController != null) {
...
*/
    ...
    //++[framework] For max auto brightness feature
    private static boolean isValidAutoBrightnessPercentage(int value) {
        return value >= PowerManager.AUTO_BRIGHTNESS_MIN_PERCENTAGE_RANAGE && value <= PowerManager.AUTO_BRIGHTNESS_MAX_PERCENTAGE_RANAGE;
    }

    @Override // Binder call
    public void setTemporaryScreenAutoBrightnessPercentageSettingOverride(int percentage) {
        mContext.enforceCallingOrSelfPermission(android.Manifest.permission.DEVICE_POWER, null);

        // ++ [framework] Add debug message.
        if (DEBUG_ON) {
            Slog.d(TAG, "setTSABPSO:(" + percentage
                    + "," + Binder.getCallingPid()
                    + "," + Binder.getCallingUid() + ")");
        }
        // -- [framework] Add debug message.
        ...
        final long ident = Binder.clearCallingIdentity();
        try {
            setTemporaryScreenAutoBrightnessPercentageSettingOverrideInternal(percentage);
        } finally {
            Binder.restoreCallingIdentity(ident);
        }
    }

    private void setTemporaryScreenAutoBrightnessPercentageSettingOverrideInternal(int percentage) {
        synchronized (mLock) {
            final boolean isOffMode = mDisplayPowerRequest.screenState == DisplayPowerRequest.SCREEN_STATE_OFF;
            final boolean isDimMode = mDisplayPowerRequest.screenState == DisplayPowerRequest.SCREEN_STATE_DIM;
            if (isOffMode || isDimMode) {
                Slog.d(TAG, "setTSABPSOI: Skip.");
                return;
            }

            if (mTemporaryScreenAutoBrightnessPercentageSettingOverride != percentage) {
                mTemporaryScreenAutoBrightnessPercentageSettingOverride = percentage;
                mDirty |= DIRTY_SETTINGS;
                updatePowerStateLocked();
            }
        }
...
//        }
        ...
        // ++ [framework] Change dim timeout during Mirror or Miracast mode
        protected void enableMirrorMode(boolean enable) {

            if (DEBUG_ON) {
                Slog.d(TAG, "enableMirrorMode = " + enable);
            }
        ...
        protected void enableMiracastMode(boolean enable) {

            if (DEBUG_ON) {
                Slog.d(TAG, "enableMiracastMode = " + enable);
            }
        ...
        protected void setProximityStateWithScreenOff(boolean value) {
            ...
            // ++ [framework] Force screen on by pressing power key.
            if (value == false) {
                PowerManagerService.this.clearForceScreenOnFlag();
                // ++ [framework] Workaround for turning on screen when screen is off.
                PowerManagerService.this.clearTurnOnScreenOnceFlag();
                // -- [framework] Workaround for turning on screen when screen is off.
            }
            // -- [framework] Force screen on by pressing power key.
        }
        ...
        // ++ [framework] Workaround for M7 p-sensor problem after ending call
        protected boolean isActualScreenOn() {
            return PowerManagerService.this.isActualScreenOn();
        }

        protected void clearDelayScreenOn() {
            PowerManagerService.this.clearDelayScreenOn();
        }

        protected void forceEnterSleepMode() {
            PowerManagerService.this.forceEnterSleepMode();
        }
        // -- [framework] Workaround for M7 p-sensor problem after ending call
    ...
    // ++ [framework] Workaround for M7 p-sensor problem after ending call
    private boolean needDelayScreenOnAfterEndCallLocked() {

        if (!mSystemReady) {
            if (DEBUG_ON) {
                Slog.w(TAG, "needDSOAECL 1");
            }
            return false;
        }
        ...
        if (mDisplayPowerController == null) {
            if (DEBUG_ON) {
                Slog.w(TAG, "needDSOAECL 2");
            }
            return false;
        }

        if (!mDisplayPowerController.isProximitySensorAvailable()) {
            if (DEBUG_ON) {
                Slog.w(TAG, "needDSOAECL 3");
            }
            return false;
        }

        return mDisplayPowerController.needDelayScreenOnAfterEndCall();
    }

    private void clearDelayScreenOn() {
        if (mDisplayPowerRequest != null) {
            mDisplayPowerRequest.delayScreenOn = false;
            if (DEBUG_ON) {
                Slog.d(TAG, "clear flag dso");
            }
        }
        mProximityPositive = false;
    }

    private void forceEnterSleepMode() {
        if (DEBUG_ON) {
            Slog.d(TAG, "force ESM");
        }
        goToSleep(SystemClock.uptimeMillis(), PowerManager.GO_TO_SLEEP_REASON_TIMEOUT);
    }
    // -- [framework] Workaround for M7 p-sensor problem after ending call

    //++[framework] To inform End Call
    @Override // Binder call
    public void informEndCall() {
        mContext.enforceCallingOrSelfPermission(com.htc.framework.Manifest.permission.APP_PLATFORM, "informEndCall"); //[framework] add permission to protect, and com.android.phone has this permission, freya, 20131112
        if (DEBUG_ON) {
            Slog.d(TAG, "inform EC");
        }
        ...
        try {
            ...
            synchronized (mLock) {
                if (shouldUseProximitySensorLocked()) {
                    if (needDelayScreenOnAfterEndCallLocked()) {
                        if (mDisplayPowerRequest != null) {
                            mDisplayPowerRequest.delayScreenOn = true;
                        }
                    }
                }
            }
    ...
    public void informOOBEStatusChange(boolean status) {
        mContext.enforceCallingOrSelfPermission(com.htc.framework.Manifest.permission.APP_PLATFORM, "informOOBEStatusChange"); //[framework] add permission to protect, freya, 20131112
        if (DEBUG_ON) {
    ...
    //++[framework] To check if P-sensor wake lock is held
    @Override // Binder call
    public boolean getProximityWakeLockIsHeld() {
        return shouldUseProximitySensorLocked();
    }
    //--[framework] To check if P-sensor wake lock is held

    // ++ [framework] Workaround for turning on screen when screen is off.
    private void clearTurnOnScreenOnceFlag() {
        if (mDisplayPowerRequest != null) {
            mDisplayPowerRequest.turnOnScreenOnce = false;
        }
    }

    private boolean needTurnOnScreenByPowerKey() {

        if (!mBootCompleted || !mSystemReady) {
            if (DEBUG_ON) {
                Slog.w(TAG, "nTOSBPK 1");
            }
            return false;
        }

        if (mActualScreenOn == false) {
            return true;
        }
